<footer class="bg-white px-4 py-5 shadow px-4 sm:px-6 md:px-8 flex justify-center border-t border-gray-200 justify-between items-center" x-data="{ privacy: false, tos: false }">
    <section>
        ...
        @else
            <span href="https://invoice.theredsun.org" target="_blank" class="text-xs md:text-sm text-gray-700">
                {{ ctrans('texts.copyright') }} &copy; {{ date('Y') }}
                <a class="text-primary hover:underline" href="https://invoice.theredsun.org" target="_blank">Red Invoice</a>.
            </span>
        @endif
    ...
    @if(auth()->guard('vendor')->user()->user && !auth()->guard('vendor')->user()->user->account->isPaid())
        <a href="https://invoice.theredsun.org" target="_blank">
            <img class="h-8" src="{{ asset('images/invoiceninja-black-logo-2.png') }}" alt="Red Invoice Logo">
        </a>
    @endif

...
class RecurringInvoiceItemReportController extends BaseController
{
    ...
    private string $filename = 'recurring_invoice_items.csv';
    ...
    /**
     * @OA\Post(
     *      path="/api/v1/reports/recurring_invoice_items",
     *      operationId="getRecurringInvoiceItemReport",
     *      tags={"reports"},
    ...
    public function __invoke(GenericReportRequest $request)
    {
        ...
        if ($request->has('send_email') && $request->get('send_email') && $request->missing('output')) {
            SendToAdmin::dispatch($user->company(), $request->all(), RecurringInvoiceItemExport::class, $this->filename);
